...
function khmerBelow100(n) {
  ...
  if (n < 20) return n === 10 ? "ដប់" : "ដប់" + ones[n - 10];
...
function numberToKhmer(value) {
  ...
  if (value === "0") return "សូន្យ";
  if (value.length > 21) return "លេខធំពេកសម្រាប់កម្មវិធីនេះ";
...
function updateRielOutput(words) {
  if (!rielOutput) initRielOutput();
  if (!rielOutput) return;

  const value = String(words || "").trim();

  rielOutput.textContent = value
    ? `${value}រៀលគត់`
    : "លទ្ធផលជារៀលគត់នឹងបង្ហាញនៅទីនេះ";
}
...
function renderCalendar() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    if (holidays[iso]) cell.classList.add("holiday");
...
function openCalendar() {
  ...
  document.getElementById("dateDisplay").classList.add("open");
  document.getElementById("dateDisplay").setAttribute("aria-expanded", "true");
...
function closeCalendar() {
  ...
  document.getElementById("dateDisplay").setAttribute("aria-expanded", "false");
}
...
function loadMomentKH() {
  ...
  if (momentKHReady) return momentKHReady;
